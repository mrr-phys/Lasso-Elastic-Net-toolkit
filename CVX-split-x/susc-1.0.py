# -*- coding: utf-8 -*-
"""
Created on Tue Oct 13 14:44:20 2015
...
if __name__=='__main__':
   ...
   # print xsparse
            for h in range(20):
                ...
                xpzro = cvx_l1(H, y, p, i, lamda1, lamda2, sig2)
            ...
            #du_list=np.zeros(5)
                ...
                for j in range(5):
                    ...
                    xp = cvx_l1(H, y, p, i, lamda1, lamda2, sig2)
                    du_list[j]+=xp[i]-xpzro[i]#avg(xp-xpzro)
                    p_list[j]=p
                    p+=.02/np.sqrt(m)
        
            chi_avg,b1= np.polyfit(p_list, du_list/np.size(du_list), 1)
        ...
        with open("susc.txt", "a") as myfile:
            ...
            myfile.write(str(m/200.)+' ' +str(np.sum(chi_list)/np.size(chi_list))+'\n')
            myfile.close()
