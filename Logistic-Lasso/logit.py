...
if __name__=='__main__':
   ...
   # a  = H>1.
    ...
    #mean = H.mean(axis=0)
    #mirror = -np.eye(m)*mean
    

    H=H[np.random.randint(H.shape[0],size=400),:]  #sub-sampling
    m, n = H.shape

    center = np.zeros((1,n))
    y = np.vstack((np.ones((m, 1),dtype=H.dtype), np.zeros((1, 1),dtype=H.dtype)))
    H = np.vstack((H,center))
    m, n = H.shape
    
    #print H.mean(axis=0)
    #y = np.vstack((np.ones((m, 1),dtype=H.dtype), np.zeros((m, 1),dtype=H.dtype)))
  ...
  #  H = H-mean  #takeout raw mean  
   ...
   # print H.shape
    ...
    while(j<100):# and logit_test < logit_accuracy):
        ...
        h=0
        x0_est = x0_current + (4./m)*np.sum(y-pi_current*(2.*h+1.)+h)
        x0_current = x0_est
        ...
        for a in xrange(H.shape[1]):
            ...
            z = x0_current+ Hdotx_current + 4.*(y-pi_current*(2.*h+1.)+h)
    ...
    colors = ['blue','red']
    x_est_sort = np.sort(x_est)[::-1]
    x_est_argsort = np.argsort(x_est)[::-1]
    SNPS_sort = [SNPS[0][i] for i in x_est_argsort]


    plt.plot(range(23),x_est_sort*100, 'o', color= 'gray')

    # Rotate x-labels on the x-axis
    fig.autofmt_xdate()

    # Label-values for x and y axis
    plt.xticks(range(23),SNPS_sort)
